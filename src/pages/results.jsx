...
const Results = () => {
  ...
  // Logic tạo mã QR VietQR (Khôi phục từ bản cũ)
  const getQrData = () => {
      ...
      <div className="bg-white rounded-3xl shadow-xl p-6 border border-slate-100">
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-2xl transition-all flex items-center justify-center gap-2">
        ...
        <div className="space-y-6 animate-fade-in-up">
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-3xl p-1 shadow-xl">
            <div className="bg-white rounded-[22px] p-6">
              ...
              {!result.ketqua && !result.nghe && !result.noi && !result.doc && !result.viet ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center">
                    <span className="text-sm font-bold text-slate-500">KẾT QUẢ CHUNG:</span>
                    <span className="text-xl font-black text-blue-700 uppercase">{result.ketqua || 'Chưa xếp loại'}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* PHẦN THANH TOÁN QRCODE (ĐÃ KHÔI PHỤC) */}
            ...
            <div className="bg-orange-50 rounded-3xl p-6 border border-orange-200 shadow-sm">
                ...
                <div className="flex flex-col md:flex-row gap-6 items-center">
                    ...
                    <div className="flex-shrink-0 bg-white p-3 border border-orange-200 rounded-2xl shadow-sm text-center">
                        ...
                        <p className="text-[10px] text-slate-400 mt-2 italic">Mở App Ngân hàng quét mã để thanh toán</p>
                    </div>
                    {/* Cột thông tin chữ */}
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="bg-white p-3 rounded-xl border border-orange-100">
                            ...
                            <p className="text-[10px] text-slate-400 mt-2 leading-relaxed">
                                * Vui lòng giữ nguyên nội dung chuyển khoản để hệ thống tự động nhận diện.
                            </p>
                        </div>
                    </div>
                </div>
          ...
          {/* Badge trạng thái nếu đã thanh toán */}
          {result.TrangThai === 'paid' && (
            <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-200 flex items-center justify-center gap-3">
               <CheckCircle2 className="text-emerald-600" size={28} />
               <span className="text-emerald-800 font-bold text-lg uppercase tracking-wider">Đã hoàn thành đóng lệ phí thi</span>
            </div>
          )}
        </div>
      )}
